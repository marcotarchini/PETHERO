<div class="wrapper row1">
  <header id="header" class="clear"> 
    <div id="logo" class="fl_left">
      <h1>MENU</h1>
    </div>
    <nav id="mainav" class="fl_right">
      <ul class="clear">
        <li class="active"><a class="drop" href="#">OPCIONES</a>
          <ul>
            <li><a href="<?php echo FRONT_ROOT . "Owner/ShowAddView"?>">INGRESAR DUEÑO</a></li>
            <li><a href="<?php echo FRONT_ROOT . "Owner/ShowView"?>">VISTA DUEÑO</a></li>
            <li><a href="<?php echo FRONT_ROOT . "Keeper/ShowListView"?>">LISTA DE GUARDIANES</a></li>
            <li><a href="<?php echo FRONT_ROOT . "Dog/ShowAddView"?>">INGRESAR MASCOTA</a></li>
            <li><a href="<?php echo FRONT_ROOT . "Booking/ShowAddView"?>">RESERVAR</a></li>
            <li><a href="<?php echo FRONT_ROOT . "Home/Logout"?>">CERRAR SESIÓN</a></li>
        
          </ul>
    </nav>
  </header>
</div>